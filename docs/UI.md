...
The popup shows compact battery values (`82%`) in the Left/Right/Case grid, or one headset value. Missing, invalid, stale-unavailable, or unknown values render as a gray em dash (`—`), never as a green battery. Zero is shown only when measured.

**Audio mode** and supported **Noise control** choices are inline native `St.Button` segments. Each `St.Button` is an `St.Bin` with one `St.BoxLayout` child containing its icon and label. The observed PipeWire profile controls the checked state and a click restores that state until a status readback, preventing optimistic hardware state. A2DP maps to Music; `headset-head-unit`, HFP, and HSP map to Meeting; unknown profiles select neither.
...
`extension/stylesheet.css` is extension-scoped. It provides hierarchy, compact spacing, muted selected-state contrast, gray unavailable batteries, neutral Connect styling, notices, truncation, and insensitive states without changing the global GNOME theme. Focus uses supported `border` and `box-shadow` properties rather than CSS `outline`.
...
- `node --test tests/frontend*.test.mjs` covers schema/parser validation, generic public labels, literal argv preservation, disconnected section gating, no-invented battery values, gray unavailable battery widgets, observed-profile selection, short notices, diagnostics containment, stale/disconnected gates, one-child `St.Bin` content, timeout/cleanup, and mocked controller lifecycle. These controller tests use fakes and are **not** a real Shell load.
